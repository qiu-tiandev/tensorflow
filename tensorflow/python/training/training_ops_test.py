...
class TrainingOpsTest(TensorFlowTestCase):
  ...
  @test_util.run_in_graph_and_eager_modes
  def testSparseApplyFtrlScalarGrad(self):
    with ops.device("/cpu:0"):
      var = variables.Variable([1.0, 2.0])
      ...
      def run_op(grad_t):
        gen_training_ops.resource_sparse_apply_ftrl(
            var.handle, accum.handle, linear.handle, grad_t, indices, lr, l1, l2, lr_power
        )
      with self.assertRaisesRegex((errors.InvalidArgumentError, ValueError), r"Argument grad must not be a scalar"):
        run_op(grad)
  ...
  def testApplyAdadeltaInvalidAccumUpdateShape(self):
    with ops.device("/cpu:0"):
      ...
      with self.assertRaisesRegex((errors.InvalidArgumentError, ValueError), r"var and accum_update do not have the same shape"):
        gen_training_ops.resource_apply_adadelta(
            var.handle, accum.handle, accum_update.handle, lr, rho, epsilon, grad
        )
  ...
  def testApplyAdamWithAmsgradInvalidVhatShape(self):
    with ops.device("/cpu:0"):
      ...
      with self.assertRaisesRegex((errors.InvalidArgumentError, ValueError), r"var and vhat do not have the same shape"):
        gen_training_ops.resource_apply_adam_with_amsgrad(
            var.handle, m.handle, v.handle, vhat.handle, beta1_power, beta2_power, lr, beta1, beta2, epsilon, grad
        )
