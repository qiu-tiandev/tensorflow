...
TEST(FfiTest, RecordStageCreateAndUpdateCommands) {
  ...
  auto handler = Ffi::BindRecord().Ctx<RecordContext>().Ctx<RecordAction>().To(
      [&](RecordContext ctx, RecordAction action) -> absl::Status {
  ...
  const auto fill_record_frame =
      [&](XLA_FFI_CallFrame* raw_call_frame, XLA_FFI_RecordAction action,
          XLA_FFI_RecordFrame& record_frame,
          XLA_FFI_RecordFrame_Extension& record_frame_ext) {
        record_frame = {raw_call_frame,
  ...
  // Create a record.
  {
    ...
    auto configure = [&](XLA_FFI_CallFrame* raw_call_frame) {
      fill_record_frame(raw_call_frame, XLA_FFI_RecordAction_Create,
                        record_frame, record_frame_ext);
    };
    auto status = Invoke(Api(), *handler, call_frame, /*context=*/{},
                         ExecutionStage::kRecord, std::move(configure));
    ASSERT_OK(status);
  ...
  // Update the record.
  {
    ...
    auto configure = [&](XLA_FFI_CallFrame* raw_call_frame) {
      fill_record_frame(raw_call_frame, XLA_FFI_RecordAction_Update,
                        record_frame, record_frame_ext);
    };
